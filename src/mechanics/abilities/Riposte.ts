...
registerAbility({
    ...
    onDamageDealt: (state, target, amount) => {
        ...
        return {
            damageDealt: [...state.damageDealt, { target: 'enemy', amount: val, source: 'Riposte' }],
            logs: addLog(state.logs, { round: state.round, message: `Riposte! Inflicted ${val} damage back.`, type: 'damage-enemy' })
        };
    }
});
